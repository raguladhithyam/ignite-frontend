...
// Helper function to check if current time is within session time range
...
router.get('/', async (req, res) => {
  try {
    ...
    // ADMIN has no restrictions - can see all records
...
router.post('/mark', requireAdminOrBrigadeLead, [
  ...
  try {
    ...
    // Check permissions for brigade leads only (ADMIN has full access)
    if (req.user.role === 'BRIGADE_LEAD') {
      const brigades = await prisma.brigade.findMany({
        where: { leaderId: req.user.id }
      });
    ...
    // Time restrictions apply only to BRIGADE_LEAD, not ADMIN
    if (req.user.role === 'BRIGADE_LEAD') {
      const now = new Date();
      ...
      console.log('Time validation debug (Brigade Lead):', {
        currentTime,
        currentDate,
        eventDate,
        session,
        eventDay: {
          fnStartTime: eventDay.fnStartTime,
          fnEndTime: eventDay.fnEndTime,
          anStartTime: eventDay.anStartTime,
          anEndTime: eventDay.anEndTime,
          fnEnabled: eventDay.fnEnabled,
          anEnabled: eventDay.anEnabled
        }
      });

      if (currentDate === eventDate) {
        if (session === 'FN') {
          if (!isTimeWithinRange(currentTime, eventDay.fnStartTime, eventDay.fnEndTime)) {
            return res.status(400).json({ 
              error: `Forenoon attendance can only be marked between ${eventDay.fnStartTime} - ${eventDay.fnEndTime}. Current time: ${currentTime}` 
            });
          }
        } else if (session === 'AN') {
          if (!isTimeWithinRange(currentTime, eventDay.anStartTime, eventDay.anEndTime)) {
            return res.status(400).json({ 
              error: `Afternoon attendance can only be marked between ${eventDay.anStartTime} - ${eventDay.anEndTime}. Current time: ${currentTime}` 
            });
          }
        }
      }
    }
    // ADMIN can mark attendance at any time without restrictions
    ...
    logger.info(`Attendance marked: ${student.tempRollNumber} - ${session} - ${status} by ${req.user.email} (${req.user.role})`);
...
router.post('/bulk-mark', requireAdminOrBrigadeLead, [
  ...
  try {
    ...
    if (req.user.role === 'BRIGADE_LEAD') {
      ...
      console.log('Bulk attendance time validation debug (Brigade Lead):', {
        currentTime,
        currentDate,
        eventDate,
        session,
    ...
    // ADMIN can mark bulk attendance at any time without restrictions
    ...
    logger.info(`Bulk attendance marked: ${studentIds.length} students - ${session} - ${status} by ${req.user.email} (${req.user.role})`);
    res.json({ 
...
router.get('/summary/:eventDayId', async (req, res) => {
  try {
    ...
    // ADMIN has no restrictions - can see all summaries
...
router.delete('/:recordId', requireAdminOrBrigadeLead, async (req, res) => {
  try {
    const { recordId } = req.params;
    ...
    if (req.user.role === 'BRIGADE_LEAD') {
      ...
      if (!brigadeIds.includes(record.student.brigadeId)) {
    ...
    logger.info(`Attendance record deleted: ${record.id} by ${req.user.email} (${req.user.role})`);
